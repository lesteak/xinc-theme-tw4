...
function title() {
  if (is_home()) {
    ...
    return __('Latest Posts', 'xinc-theme');
  }
  ...
  if (is_search()) {
    return sprintf(__('Search Results for %s', 'xinc-theme'), get_search_query());
  }

  if (is_404()) {
    return __('Not Found', 'xinc-theme');
  }
 ...
 *
 * @since 0.0.2
 * @param type $str - name, time in seconds, function
 * @return null
 */
